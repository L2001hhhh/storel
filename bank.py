...
# 银行字典
bank = {}  # 相当于一个空的数据库
# bank_name = "中国人民银行"  # 全局变量

# 银行开户逻辑
def bank_adduser(account, username, password, country, province, street, door, money, registeDate, bankname):
...
# 查询用户名是否重复
    ...
    param = [account, username, password, country, province, street, door, money, registeDate, bankname]
    update(sql, param)
    return 1
...
def useradd():
    ...
    money = 0
    registeDate=datetime.datetime.now()
    bankname=input("请输入你的开户行：")
    bankadd = bank_adduser(account, username, password, country, province, street, door, money, registeDate, bankname)
    if bankadd == 1:
        ...
        info = '''
                ...
                '''
        # 每个元素都可传入%
        print(info % (username, account, country, province, street, door, money, registeDate, bankname))
    elif bankadd == 2:
        print("用户已存在，请勿重复添加！")
    elif bankadd == 3:
        print("数据库已满")
...
def bank_outputMoney(account_get, password_get, output_money):
    ...
    # 判断密码是否正确
    sql7="select password from bank where account=%s"
    param7=[account_get]
...
def outputMoney():
    ...
    if withdrawals == 1:
        ...
        # 取款后金额
        sql8 = "update bank set money=money-%s where account=%s"
        param8 = [output_money,account_get]
        update(sql8, param8)
        # 查询余额
        sql10="select money from bank where account=%s"
        param10=[account_get]
...
def bank_transfer(out_account, in_account, out_money):
    # 判断转出的账号是否存在
    sqla="select * from bank where account=%s"
    parama=[out_account]
    datea=select(sqla,parama,mode="one")[0]
    # 判断转入的账号是否存在
    sqlb="select * from bank where account=%s"
    paramb=[in_account]
    dateb=select(sqlb,paramb,mode="one")[0]
    # 判断转出的账号密码是否正确
    slqc="select password from bank where account=%s"
    paramc=[out_account]
    datec=select(slqc,paramc,mode="all")[0]
    # 判断当前余额是否大于转出的金额
    # 当前余额
    sqld="select money from bank where account=%s"
    paramd=[out_account]
...
def transfer():
    ...
    if Transfer == 1:
        ...
        # 转账后转出账号金额
        sqle="update bank set money=money-%s where account=%s"
        parame=[out_money,out_account]
        update(sqle,parame)
        # 查询转出账号的余额
        sqlqq="select money from bank where account=%s"
        paramqq=[out_account]
        dateqq=select(sqlqq,paramqq,mode="one")[0]
        print("转账成功！")
        print("转出账号余额为：", dateqq)
        # 转账后转入账号余额
        sqlww="update bank set money=money+%s where account=%s"
        paramww=[out_money,in_account]
        update(sqlww,paramww)
        # 查询转入账号余额
        sqlf="select money from bank where account=%s"
        paramf=[in_account]
...
def search():
    ...
    # 判断查询的账号是否存在
    sqlg="select *from bank where account=%s"
    paramg=[account_src]
    dateg=select(sqlg,paramg,mode="all")[0]
    # 查询账号的密码是否正确
    sqlh="select password from bank where account=%s"
    paramh=[account_src]
    dateh=select(sqlh,paramh,mode="one")[0]
    if len(dateg)>0:
        if len(dateh)>0:
             #查询账号信息
            sqli="select * from bank where account=%s"
            parami=[account_src]
